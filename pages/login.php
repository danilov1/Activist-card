...
if($_POST['act'] == 'logout') {
	if($_COOKIE['a']) {
		$_logoutsql = "DELETE FROM `".$GLOBALS['config_db']['mysql_db']."`.`tokens` WHERE `tokens`.`token`='".$_COOKIE['a']."';";
...
if($_POST['act'] == 'access_token') {
	...
	$pregetaccess = mysql_query("SELECT `id`,`sin`,`password`,`type`,`access` from `users` WHERE `sin`='".$_POST['l']."' AND `access` ='y' LIMIT 1");
	$getaccess = mysql_fetch_row($pregetaccess);
	if(!$getaccess[0]) { errorjson("access_wrong"); }
	if(md5($_POST['p']) !== $getaccess[2]) { errorjson("access_wrong"); }
	...
	$findtokens = "DELETE FROM `".$GLOBALS['config_db']['mysql_db']."`.`tokens` WHERE `tokens`.`user`='".$getaccess[0]."';";
